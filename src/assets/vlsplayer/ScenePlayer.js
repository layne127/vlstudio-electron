...
const curve = require('./js/curves/curve');
const splineCurve = require('./js/curves/splineCurve');
const cameraPath = require('./js/animation/cameraPath');
const cameraPathAnimation = require('./js/animation/cameraPathAnimation');
const cameraFollowAnimationy = require('./js/animation/cameraFollowAnimation');
OBJModel(xeo);
...
export default function(canvasId, transparent = false) {
// console.log('vls', JSON.parse(vls));
    ...
    new xeogl.Mesh(
        ...
        material: new xeogl.PhongMaterial(scene, {
            ...
            diffuseMap: new xeogl.Texture(scene, {
              //  src: '../../assets/vlsplayer/textures/diffuse/UVCheckerMap11-1024.png',
              src: 'assets/vlsplayer/textures/diffuse/UVCheckerMap11-1024.png',
                scale: [-5.0, 5.0]
            }),
            xalpha: 0.99,
    ...
    this.load = function (data, url) {
        ...
        var model = loadModel(m.id, url + '/' + m.src);
        //    var model = loadModel(m.id, `${window.location.origin}` + m.src);
    ...
    this.clearModels = function () {
        Object.values(scene.models).forEach(m => {
            m.destroy();
        })
    }
    this.reset = function() {
